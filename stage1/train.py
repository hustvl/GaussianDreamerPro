...
from utils.loss_utils import l1_loss, ssim, tv_loss,NormalLoss
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    if opt.lambda_normal > 0:
        viewpoint_camera = scene.getRandTrainCameras()[0]
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}",
                                          "gs number": f"{gaussians._xyz.shape[0]}"})
            ...
            # Densification
            if iteration < opt.densify_until_iter and gaussians._xyz.shape[0]<=300_0000:
                # Keep track of max radii in image-space for pruning
                gaussians.max_radii2D[visibility_filter] = torch.max(gaussians.max_radii2D[visibility_filter], radii[visibility_filter])
                viewspace_point_tensor_grad = torch.zeros_like(visibility_filter_list[0])
                for idx in range(len(visibility_filter_list)):
                    viewspace_point_tensor_grad = viewspace_point_tensor_grad + visibility_filter_list[idx].grad
...
if __name__ == "__main__":
    ...
    parser.add_argument("--initprompt", type=str, default = None)
    ...
    gp.text = args.prompt
    if args.initprompt is not None:
        gcp.init_prompt = args.initprompt
    else:
        gcp.init_prompt = args.prompt
