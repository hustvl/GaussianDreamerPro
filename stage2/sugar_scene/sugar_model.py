...
from diff_gaussian_rasterization_2dgs import GaussianRasterizationSettings as GaussianRasterizationSettings2dgs
from diff_gaussian_rasterization_2dgs import GaussianRasterizer as GaussianRasterizer2dgs
...
from sugar_scene.cameras import CamerasWrapper, convert_camera_from_gs_to_pytorch3d
from lucidscene.dataset_readers import sceneLoadTypeCallbacks, GenerateRandomCameras, GeneratePurnCameras, GenerateCircleCameras
import numpy as np
...
def uniform_barycentric_coordinates_torch(n, device):
    """
    使用PyTorch生成均匀分布的重心坐标。
    n: 小三角形的数量，应该是完全平方数。
    """
    side_divisions = int(torch.sqrt(torch.tensor(n, dtype=torch.float)))
    step = 1 / side_divisions
    coords = []

    for i in range(int(side_divisions)):
        for j in range(int(side_divisions) - i):
            # 计算每个小三角形重心的重心坐标
            a = (i + 0.5) * step
            b = (j + 0.5) * step
            c = 1 - a - b
    ...
    return torch.tensor(coords, dtype=torch.float32, device = device)[..., None]
...
class SuGaR(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs) -> None:
        ...
        if surface_mesh_to_bind is not None:
            ...
            surface_mesh_to_bind = surface_mesh_to_bind.subdivide_midpoint(number_of_iterations = 1)
            surface_mesh_to_bind = surface_mesh_to_bind.simplify_quadric_decimation(target_number_of_triangles = 40000)
            ...
            if n_gaussians_per_surface_triangle >= 6:
                self.surface_triangle_circle_radius = np.sqrt(3.)/6.0/np.sqrt(n_gaussians_per_surface_triangle)*2.0
                self.surface_triangle_bary_coords = uniform_barycentric_coordinates_torch((int(np.sqrt(n_gaussians_per_surface_triangle*2.0)))**2, device=nerfmodel.device)
        ...
        if self.binded_to_surface_mesh and (not learn_surface_mesh_opacity):
            ...
            all_densities = inverse_sigmoid(0.7 * torch.ones((n_points, 1), dtype=torch.float, device=points.device))
        ...
        if not self.binded_to_surface_mesh:
            ...
            scales = scales.clamp_min(0.0000001).reshape(len(faces_verts), -1, 1).expand(-1, self.n_gaussians_per_surface_triangle, 3).clone().reshape(-1, 3)
            self._scales = nn.Parameter(
    ...
    def compute_level_surface_points_from_camera_efficient(
        ...
        if use_gaussian_depth:
            point_depth = p3d_cameras.get_world_to_view_transform().transform_points(self.points)[..., 2:].expand(-1, 3)
            depth = self.render_image_gaussian_rasterizer_2dgs( 
                    camera_indices=cam_idx,
                    bg_color=torch.Tensor([-1., -1., -1.]).to(self.device),
                    sh_deg=0,
                    compute_covariance_in_rasterizer=True,
    ...
    def compute_level_surface_points_from_camera_fast_2dgs(
        self, 
        nerf_cameras=None,
        cam_idx=0,
        rasterizer=None,
    ...
    def render_image_gaussian_rasterizer_2dgs(
        self, 
        ...
        raster_settings = GaussianRasterizationSettings2dgs(
        ...
        rasterizer = GaussianRasterizer2dgs(raster_settings=raster_settings)
        ...
        rendered_image, _, _, _, radii = rasterizer(
            means3D = positions,
            means2D = means2D,
            shs = shs,
    ...
    def render_image_gaussian_rasterizer_pose(
            ...
            if not compute_color_in_rasterizer:
                if sh_rotations is None:
                    ...
                    splat_colors = self.get_points_rgb(
                        positions=positions, 
                        camera_centers=None,
                        directions=(torch.nn.functional.normalize(positions - viewpoint_camera.camera_center, dim=-1).unsqueeze(1) @ sh_rotations)[..., 0, :],
                        sh_levels=sh_deg+1,)
                shs = None
            else:
                shs = self.sh_coordinates
        ...
        focal = 1/(2* math.tan(self.fov_x/2))
...
def extract_texture_image_and_uv_from_gaussians(
    ...
    for cam_idx in range(len(rc.nerfmodel.training_cameras)):
        ...
        # Render rgb img
        rgb_img_out = rc.render_image_gaussian_rasterizer_pose(
            pose=rc.nerfmodel.training_cameras.gs_cameras[cam_idx],
            sh_deg=rc.sh_levels-1,
