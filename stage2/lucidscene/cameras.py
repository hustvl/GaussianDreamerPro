...
import numpy as np
from lucidutils.graphics_utils import getWorld2View2, getProjectionMatrix, fov2focal, getc2w_self
...
class SCamera(nn.Module):
    def __init__(self, colmap_id, R, T, image_w, image_h,
                 FoVx, FoVy, uid, delta_polar, delta_azimuth, delta_radius, opt,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device = "cuda", SSAA=False
                 ):
        super(SCamera, self).__init__()
        ...
        self.image_width = image_w * ssaa
        self.image_height = image_h * ssaa

        self.trans = trans
        self.scale = scale

        RT = torch.tensor(getWorld2View2(R, T, trans, scale))
        self.c2w = torch.tensor(getc2w_self(R, T, trans, scale))
        self.world_view_transform = RT.transpose(0, 1).cuda()
        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0,1).cuda()
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3]
