...
from lucidutils.graphics_utils import getWorld2View2, focal2fov, fov2focal
import numpy as np
import json
from pathlib import Path
from lucidutils.pointe_utils import init_from_pointe
from plyfile import PlyData, PlyElement
from lucidutils.sh_utils import SH2RGB
from lucidutils.general_utils import inverse_sigmoid_np
# from lucidscene.gaussian_model import BasicPointCloud
...
from lucidutils.graphics_utils import BasicPointCloud
...
def readCircleCamInfo(path,opt):
    ...
    if not os.path.exists(ply_path):
        ...
        # elif opt.init_shape == 'pointe':
        #     num_pts = int(num_pts/5000)
        #     xyz_ini,rgb_ini = init_from_pointe(opt.init_prompt)
        #     xyz_ini[:,1] = - xyz_ini[:,1]
        #     xyz_ini[:,2] = xyz_ini[:,2] + 0.15
        #     xyz,rgb = add_points(xyz_ini,rgb_ini)
        #     # thetas = np.random.rand(num_pts)*np.pi
        #     # phis = np.random.rand(num_pts)*2*np.pi        
        #     # radius = np.random.rand(num_pts)*0.05
        #     # # We create random points inside the bounds of sphere
...
def GenerateRandomCameras(opt, size=1, SSAA=True):
    # random pose on the fly
