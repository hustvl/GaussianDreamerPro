...
from lucidscene import Scene as LucidScene
from random import randint
import random
from lucidutils.loss_utils import tv_loss
...
def refined_training(args):
    ...
    # num_iterations = 5_000  # Changed

    spatial_lr_scale = None
    position_lr_init=0.005
    position_lr_final=0.00025
    position_lr_delay_mult=0.05
    position_lr_max_steps=5_000
    feature_lr=0.005
    opacity_lr=0.005
    scaling_lr=0.0005
    rotation_lr=0.0001
    ...
    # Point cloud
    if initialize_from_trained_3dgs or True:
        with torch.no_grad():    
    ...
    if bind_to_surface_mesh:
        ...
        o3d_mesh_save = o3d_mesh.subdivide_midpoint(number_of_iterations = 1)
        o3d.io.write_triangle_mesh(os.path.join(args.output_dir, "now.obj"), o3d_mesh_save, write_triangle_uvs=True, write_vertex_colors=True, write_vertex_normals=True)
    ...
    # ====================SDS function====================
    parser_lucid = args.parser_lucid

   

    # args_lucid.opt = args.lucidopt

    lp = ModelParams(parser_lucid)
    opt = OptimizationParams_arg(parser_lucid)
    pp = PipelineParams(parser_lucid)
    gcams = GenerateCamParams(parser_lucid)
    guidance_opt = GuidanceParams(parser_lucid)
    args_lucid = parser_lucid.parse_args(sys.argv[1:])
    args_lucid.opt = args.lucidopt

    with open(args_lucid.opt) as f:
        opts = yaml.load(f, Loader=yaml.FullLoader)
    lp.load_yaml(opts.get('ModelParams', None))
    opt.load_yaml(opts.get('OptimizationParams', None))
    pp.load_yaml(opts.get('PipelineParams', None))
    gcams.load_yaml(opts.get('GenerateCamParams', None))
    guidance_opt.load_yaml(opts.get('GuidanceParams', None))

    lp.opt_path = args_lucid.opt
    args_lucid.port = opts['port']
    args_lucid.save_video = opts.get('save_video', True)
    args_lucid.seed = opts.get('seed', 0)
    args_lucid.device = opts.get('device', 'cuda')

    # override device
    guidance_opt.g_device = args_lucid.device
    lp.data_device = args_lucid.device
    gcams.device = args_lucid.device

    guidance_opt.text = args.prompt
    gcams.init_prompt = args.prompt



    guidance, embeddings = guidance_setup(guidance_opt)   
    lucidscene = LucidScene(lp, gcams)
    viewpoint_stack = None
    use_control_net = False
    save_folder = os.path.join(args.output_dir,"train_process/")
    ...
    train_losses = []
    t0 = time.time()
    sugar.image_height = gcams.image_h
    sugar.image_width = gcams.image_w
    ...
    for batch in range(9_999_999):
        ...
        if not opt.use_progressive:                
            if iteration >= opt.progressive_view_iter and iteration % opt.scale_up_cameras_iter == 0:
                lucidscene.pose_args.fovy_range[0] = max(lucidscene.pose_args.max_fovy_range[0], lucidscene.pose_args.fovy_range[0] * opt.fovy_scale_up_factor[0])
                lucidscene.pose_args.fovy_range[1] = min(lucidscene.pose_args.max_fovy_range[1], lucidscene.pose_args.fovy_range[1] * opt.fovy_scale_up_factor[1])

                lucidscene.pose_args.radius_range[1] = max(lucidscene.pose_args.max_radius_range[1], lucidscene.pose_args.radius_range[1] * opt.scale_up_factor)
                lucidscene.pose_args.radius_range[0] = max(lucidscene.pose_args.max_radius_range[0], lucidscene.pose_args.radius_range[0] * opt.scale_up_factor)

                lucidscene.pose_args.theta_range[1] = min(lucidscene.pose_args.max_theta_range[1], lucidscene.pose_args.theta_range[1] * opt.phi_scale_up_factor)
                lucidscene.pose_args.theta_range[0] = max(lucidscene.pose_args.max_theta_range[0], lucidscene.pose_args.theta_range[0] * 1/opt.phi_scale_up_factor)

                # opt.reset_resnet_iter = max(500, opt.reset_resnet_iter // 1.25)
                lucidscene.pose_args.phi_range[0] = max(lucidscene.pose_args.max_phi_range[0] , lucidscene.pose_args.phi_range[0] * opt.phi_scale_up_factor)
                lucidscene.pose_args.phi_range[1] = min(lucidscene.pose_args.max_phi_range[1], lucidscene.pose_args.phi_range[1] * opt.phi_scale_up_factor)
                
                print('scale up theta_range to:', lucidscene.pose_args.theta_range)
                print('scale up radius_range to:', lucidscene.pose_args.radius_range)
                print('scale up phi_range to:', lucidscene.pose_args.phi_range)
                print('scale up fovy_range to:', lucidscene.pose_args.fovy_range)
        ...
        for i in range(C_batch_size):
            ...
            try:
                ...
                viewpoint_stack = lucidscene.getRandTrainCameras().copy()
                viewpoint_cam = viewpoint_stack.pop(randint(0, len(viewpoint_stack)-1))
            
            azimuth_all.append(torch.tensor(viewpoint_cam.delta_azimuth, device = sugar.device))
            polar_all.append(torch.tensor(viewpoint_cam.delta_polar, device = sugar.device))
            radius_all.append(torch.tensor(viewpoint_cam.delta_radius, device = sugar.device))
            c2w_all.append(torch.tensor(viewpoint_cam.c2w, device = sugar.device))
            ...
            outputs = sugar.render_image_gaussian_rasterizer_pose( 
                pose = viewpoint_cam,
                verbose=False,
                bg_color = None,
            ...
            scales.append(outputs["scales"])
            images.append(outputs["render"])
            depths.append(outputs["depth"])
            alphas.append(outputs["alpha"])
        azimuth_all = torch.stack(azimuth_all, dim=0)
        polar_all = torch.stack(polar_all, dim=0)
        radius_all = torch.stack(radius_all, dim=0)
        c2w_all = torch.stack(c2w_all, dim=0)
        ...
        images512 = F.interpolate(images, (512,512), mode = 'bilinear', align_corners = False)
        depths512 = F.interpolate(depths, (512,512), mode = 'bilinear', align_corners = False)
        alpha512 = F.interpolate(alphas, (512,512), mode = 'bilinear', align_corners = False)
        if guidance_opt.perpneg:
            loss = guidance.train_step_perpneg(torch.stack(text_z_, dim=1), images512, 
                                                pred_depth=depths512, pred_alpha=alpha512,
                                                ...
                                                weights = torch.stack(weights_, dim=1), resolution=(images512.shape[2], images512.shape[3]),
                                                guidance_opt=guidance_opt,as_latent=_aslatent, embedding_inverse = text_z_inverse)
        else:
            loss = guidance.train_step(torch.stack(text_z_, dim=1), images512, 
                                    pred_depth=depths512, pred_alpha=alpha512,
                                    grad_scale=guidance_opt.lambda_guidance,
                                    use_control_net = use_control_net ,save_folder = save_folder,  iteration = iteration, warm_up_rate=warm_up_rate, 
                                    resolution=(images512.shape[2], images512.shape[3]),
                                    guidance_opt=guidance_opt,as_latent=_aslatent, embedding_inverse = text_z_inverse)
                ...
                if regularize_sdf and iteration > start_sdf_regularization_from:
                    ...
                    if (use_sdf_estimation_loss or enforce_samples_to_be_on_surface) and iteration > start_sdf_estimation_from:
                        ...
                        if backpropagate_gradients_through_depth:                                
                            ...
                            depth = sugar.render_image_gaussian_rasterizer_pose(
                                        pose=viewpoint_cam,
        ...
        if iteration==1 or iteration % print_loss_every_n_iterations == 0:
            ...
            CONSOLE.print(f"loss: {loss.item():>7f}  [{iteration:>5d}/{num_iterations:>5d}]",
        ...
        if (iteration in [50, 100, 300, 4999] or iteration % 200 ==0):
            video_inference(iteration, lucidscene, sugar.render_image_gaussian_rasterizer_pose, current_sh_levels, save_path= args.output_dir)
...
def video_inference(iteration, scene, renderFunc, current_sh_levels, save_path = None):
    sharp = Trans.RandomAdjustSharpness(3, p=1.0)

    save_folder = os.path.join(save_path,"videos/{}_iteration".format(iteration))
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)  # makedirs 
    ...
    if not os.path.exists(images_folder):
        ...
        print('images is in :', images_folder)
    ...
    if config['cameras'] and len(config['cameras']) > 0:
        ...
        for idx, viewpoint in enumerate(config['cameras']):
            ...
            if depth is not None:
                ...
                imageio.imsave(os.path.join(images_folder,"{}_depth_view.jpg".format(idx)),np.repeat(depths, 3, axis=2))
  
            image = torch.clamp(rgb, 0.0, 1.0) 
            image = image.detach().cpu().permute(1,2,0).numpy()
            image = (image * 255).round().astype('uint8')
            img_frames.append(image)    
            imageio.imsave(os.path.join(save_folder,"{}_gs.jpg".format(idx)), image)   
